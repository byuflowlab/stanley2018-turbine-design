...
%DIF DEL original-paper.tex   Thu Oct 18 16:38:45 2018
%DIF ADD revised-paper.tex    Thu Oct 18 15:15:14 2018
...
\introduction 
Mitigating wake interactions among wind turbines is one of the most difficult challenges in wind farm design. Upstream turbines remove energy from the wind, decreasing the energy available to \DIFdelbegin \DIFdel{the rest of the farm}\DIFdelend \DIFaddbegin \DIFadd{and increasing the loading on downstream turbines}\DIFaddend . These wake losses often reduce the power production by 10--20\% when compared to unwaked conditions \citep{barthelmie2007modelling,barthelmie2009modelling,briggs2013navigating}. 
...
For large wind farms \DIFaddbegin \DIFadd{purely }\DIFaddend gradient-free methods become infeasible, and while gradient-based optimization methods converge to local minima, they scale much better with the number of design variables.
...
Three main methods exist to decrease wake interactions among wind turbines in a wind farm: layout optimization, \DIFdelbegin \DIFdel{yaw }\DIFdelend \DIFaddbegin \DIFadd{active }\DIFaddend control, and turbine design. The wind farm layout optimization problem has been widely studied in recent years. There is abundant literature from the research community discussing various methods to approach the wind farm layout optimization problem including gradient-free methods \citep{marmidis2008optimal,emami2010new,kusiak2010design,ituarte2011optimization,feng2015solving,gao2015wind} and gradient-based methods \citep{perez2013offshore,park2015layout,Fleming2016,guirguis2016toward,gebraad2017maximization}. 
%DIF > 
\DIFaddbegin \DIFadd{A mix of gradient free and gradient based optimization approaches might also be advantageous \mbox{%DIFAUXCMD
\citep{rethore2014topfarm}}\hspace{0pt}%DIFAUXCMD
.
%DIF > 
}\DIFaddend The premise of layout optimization is simple: design the wind farm layout such that wake interactions among turbines are minimal.
...
\DIFaddbegin \DIFadd{Active turbine control refers to adjusting a turbine's settings to affect the wake it produces downsteam. This will reduce the power production of the individual turbine, but may be beneficial to the farm as a whole. }\DIFaddend Wake steering through \DIFdelbegin \DIFdel{turbine }\DIFdelend \DIFaddbegin \DIFadd{active }\DIFaddend yaw control is \DIFdelbegin \DIFdel{another method to decrease wake interactions between wind turbines  }\DIFdelend \DIFaddbegin \DIFadd{a method of control optimization }\DIFaddend \citep{Fleming2016,gebraad2017maximization}. Although not considered in this paper, \DIFdelbegin \DIFdel{yaw }\DIFdelend \DIFaddbegin \DIFadd{active }\DIFaddend control can be applied to the wind farms in this study for additional improvements.
...
\DIFaddbegin \DIFadd{Additionally, we use a version of FLORIS wake model that has been modified to consider the 3D flow field in the wind farm instead of a simpler 2D flow field \mbox{%DIFAUXCMD
\citep{stanley2017gradient}}\hspace{0pt}%DIFAUXCMD
.
}\DIFaddend 

The total velocity deficit\DIFaddbegin \DIFadd{, $L$, }\DIFaddend at any given point was defined as the square root of the sum of the squares of the loss contribution from each turbine wake\DIFdelbegin \DIFdel{:
}\DIFdelend \DIFaddbegin \DIFadd{, $l_i$:
}\DIFaddend \begin{equation}
L = \DIFdelbegin \DIFdel{\sqrt{\sum_{i=1}^\text{nTurbs}L_i^2}
}\DIFdelend \DIFaddbegin \DIFadd{\sqrt{\sum_{i=1}^\text{nTurbs}l_i^2}
...
We assumed that up to rated power, the rotation of \DIFdelbegin \DIFdel{each wind turbine }\DIFdelend \DIFaddbegin \DIFadd{the blades }\DIFaddend could be controlled such that a constant power coefficient of 0.42 was achieved. The wind turbine power generation was defined as:
\begin{equation}
P = 
\begin{cases} 
...
V_{\text{eff}} = V(1-L)
\end{equation}
\noindent Where $V$ is the free stream wind speed at the turbine hub height, and L is the total velocity deficit. \DIFaddbegin \DIFadd{In the case of partial waking, an area weighted average is used to calculate $V_{\text{eff}}$.
}\DIFaddend 
...
\begin{equation}
W(\DIFdelbegin \DIFdel{x}\DIFdelend \DIFaddbegin \DIFadd{V}\DIFaddend ) = \Big(\frac{k}{V_{\text{mean}}}\Big)\Big(\DIFdelbegin \DIFdel{\frac{x}{V_{\text{mean}}}}\DIFdelend \DIFaddbegin \DIFadd{\frac{V}{V_{\text{mean}}}}\DIFaddend \Big)^{k-1}\text{exp}\Big[\Big(-\DIFdelbegin \DIFdel{\frac{x}{V_{\text{mean}}}}\DIFdelend \DIFaddbegin \DIFadd{\frac{V}{V_{\text{mean}}}}\DIFaddend \Big)^k\Big]
\end{equation}
\DIFaddbegin \DIFadd{This equation defines the frequency ($W$) of a certain wind speed ($V$).
}\DIFaddend The shape factor, $k$, was set as 1.76\DIFaddbegin \DIFadd{, which is in the range of realistic shape factors fit to real wind data from various sites around the world \mbox{%DIFAUXCMD
\citep{garcia1998fitting,lun2000study}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend . 
...
\DIFdelbegin \DIFdel{The structural analysiswas used to constrain the tower from stress or buckling issues}\DIFdelend \DIFaddbegin \DIFadd{In the structural analysis, we calculated stress and buckling in the tower and constrained these values during the optimization}\DIFaddend .
...
%DIFDELCMD < 				& \text{minimize}
%DIFDELCMD < 					& & \text{COE} \\
%DIFDELCMD <                 & \text{w.r.t.} 
%DIFDELCMD <                 	&& x_i,~ y_i,~ H_{1,2},~ D_{1,2},~ R_{1,2},~ d_{(1,j)},~ d_{(2,j)},~ t_{(1,j)},~t_{(2,j)}\\
%DIFDELCMD <                 		&&& i = 1, \ldots, n; \; j = 1,~2,~3 \\
%DIFDELCMD < 				& \text{subject to}
%DIFDELCMD < 					& & \text{boundary constraints} \\
%DIFDELCMD < 						&&& \sqrt{(x-x_i)^2+(y-y_i)^2} \geq 2D_{\text{rotor}} \\
%DIFDELCMD < 						&&& H_1-\frac{D_1}{2}, H_2-\frac{D_2}{2} \geq 10~ \text{m}\\
%DIFDELCMD <                 		&&& d_{(1,j),(2,j)} \leq 6.3~ \text{m}\\
%DIFDELCMD <                 		&&& d_{(1,top),(2,top)} \geq 3.87~ \text{m}\\
%DIFDELCMD < 						&&& \frac{3\hspace{0.08cm}\Omega}{1.1} \geq f_{1,2} \geq 1.1\hspace{0.08cm}\Omega \\
%DIFDELCMD <                 		&&& \text{shell buckling margins: max thrust} \leq 1 \\
%DIFDELCMD <                         &&& \text{shell buckling margins: survival load} \leq 1 \\
%DIFDELCMD <                 		&&& \frac{d_{(1,j)}}{t_{(1,j)}},\frac{d_{(2,j)}}{t_{(2,j)}} \geq 120\\
%DIFDELCMD <                         &&& 46~ \text{m} < D_1,D_2 < 160~ \text{m}  \\
%DIFDELCMD <                         &&& 500~ \text{kW} < R_1,R_2 < 10,000~ \text{kW} \\
...
This wind farm was optimized with wind data from the city of Alturas, California \DIFaddbegin \DIFadd{gathered by Iowa State University}\DIFaddend , shown in Fig. \ref{wind_roses} \DIFdelbegin \DIFdel{. For lack of reliable data}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{Alturas}}\hspace{0pt}%DIFAUXCMD
. For this wind rose}\DIFaddend , the average wind speed, $V_{\text{mean}}$, from each direction of this wind rose was assumed to be eight meters per second, shown in Fig \ref{wind_speeds}.
The second wind farm was \DIFaddbegin \DIFadd{based on }\DIFaddend the Princess Amalia wind farm, a real farm off the coast of the Netherlands which has 60 wind turbines, is shown in Fig. \ref{layouts}. This wind farm was optimized with the wind direction and average directional speed data from \DIFdelbegin \DIFdel{the true Princess Amalia farm}\DIFdelend \DIFaddbegin \DIFadd{NoordzeeWind meteorological mast located in the North Sea}\DIFaddend , shown in Figs. \ref{wind_roses} and \ref{wind_speeds} \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{brand2012database}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend . The farm boundary for the Princess Amalia wind farm was the convex hull of the original Princess Amalia layout. 
...
\begin{figure}[htbp]
  ...
  \caption{\label{layouts} The two different wind farm designs that were optimized. On the left is a contrived circular wind farm design with 32 turbines. On the right is the Princess Amalia wind farm, an offset grid design with 60 wind turbines. \DIFaddbeginFL \DIFaddFL{The circles representing the turbine locations have a diameter of 80 meters in this figure, equal to the baseline rotor diameter.}\DIFaddendFL }
\end{figure}
%
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.49\textwidth]{Figures/alturas_rose.pdf}
  \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.49\textwidth]{Figures/amalia_rose.pdf}
%DIFDELCMD <   %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.49\textwidth]{Figures/NZW_rose.pdf}
  \DIFaddendFL \caption{\label{wind_roses} On the left, the wind direction distribution in Alturas, California, separated into 36 bins, every 10 degrees. On the right, the wind direction distribution of the Princess Amalia Wind Farm, separated into 72 bins, every 5 degrees.}
\end{figure}
%
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.49\textwidth]{Figures/alturas_speeds.pdf}
  \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.49\textwidth]{Figures/amalia_speeds.pdf}
%DIFDELCMD <   %%%
...
\DIFaddend We also optimized each wind farm for different turbine spacings by adjusting the \DIFdelbegin \DIFdel{turbine locations by }\DIFdelend \DIFaddbegin \DIFadd{wind farm size }\DIFaddend some spacing multiplier, $\beta$\DIFdelbegin \DIFdel{, which is simply some constant multiplied to each turbine location. The wind farm boundaries were adjusted accordingly with the spacing multipliers, meaning the }\DIFdelend \DIFaddbegin \DIFadd{. The baseline turbine locations were adjusted to scale with the varied wind farm sizes. The boundary }\DIFaddend radius of the circular wind farm was \DIFdelbegin \DIFdel{also }\DIFdelend multiplied by the spacing multiplier, and the convex hull of the Princess Amalia farm was applied to the baseline turbine locations \DIFdelbegin \DIFdel{adjusted }\DIFdelend \DIFaddbegin \DIFadd{that had been multiplied }\DIFaddend by the spacing multiplier. Figure \ref{farm_spacings} shows both of the wind farms adjusted by the spacing multipliers, as well as the turbine spacing in baseline rotor diameters. \DIFaddbegin \DIFadd{The circles represent the turbine locations in this figure, with the circle diameter equal to the baseline rotor diameter, 80 meters. }\DIFaddend As the turbine designs were optimized, \DIFdelbegin \DIFdel{these spacings }\DIFdelend \DIFaddbegin \DIFadd{the spacings indicated in this figure }\DIFaddend (in rotor diameters) increased or decreased according to the new rotor diameters. Note that in the circular wind farm, the turbine distances are presented in the rows closely inline with the dominant wind direction (ten degrees south of west, see Fig. \ref{wind_roses}). The closest neighboring turbines are actually $\sqrt{2}/2$ multiplied by this value. 
