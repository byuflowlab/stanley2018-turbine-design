...
!TODO After you run, 1.) Change all nbdirs max (all one word) to nbdirs 2.) delete ! USE DIFFSIZES 3.) in florisbv, chnage all the input variables b to intent out 4.) delete wtvelocity from florisbv
...
& , ke_in
!TODO move Vinf to len(nTurbines)
...
!TODO real(dp), dimension(3, nSamples), intent(in) :: wsPositionXYZw
! local (General)
  REAL(dp), DIMENSION(nturbines) :: ke, yaw
...
! visualization
  INTEGER :: loc
!TODO real(dp), dimension(nSamples) :: velX, velY, velZ
! local (Wake centers and diameters)
! in rotor diameters
...
! visualization out
!TODO real(dp), dimension(nSamples), intent(out) :: wsArray
  INTRINSIC COS, ATAN, MAX
  INTRINSIC KIND
  INTRINSIC SIN
  INTRINSIC SUM
  ...
  REAL(dp) :: temp5
  REAL(dp) :: temp4
!TODO if (nSamples == 1) then
!TODO nSamples = 0
!TODO end if
  yaw = yawdeg*pi/180.0_dp
!TODO velX(:) = wsPositionXYZw(1, :)
!TODO velY(:) = wsPositionXYZw(2, :)
!TODO velZ(:) = wsPositionXYZw(3, :)
!!!!!!!!!!!!!!!!!!!!!!!!!!!! Wake Centers and Diameters !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  spline_bound = 1.0_dp
! calculate locations of wake centers in wind ref. frame
  wakecentersyt_mat = 0.0_dp
...
&           ) + bd*deltax
  ...
  END DO
! wake center calculations at each sample point
!TODO do loc = 1, nSamples
!TODO if (turbineXw(turb) < velX(loc)) then
!TODO deltax = velX(loc) - turbineXw(turb)
!TODO factor = (2.0_dp*kd*deltax/rotorDiameter(turb)) + 1.0_dp
!TODO wakeCentersY(loc, turb) = turbineYw(turb)
!TODO displacement = wakeAngleInit*(wakeAngleInit* &
!TODO & wakeAngleInit + 15.0_dp*factor*factor* &
!TODO factor*factor)/((30.0_dp*kd/ &
!TODO rotorDiameter(turb))*(factor*factor* &
!TODO & factor*factor*factor))
!TODO displacement = displacement - &
!TODO & wakeAngleInit*(wakeAngleInit* &
!TODO & wakeAngleInit + 15.0_dp)/(30.0_dp*kd/ &
!TODO rotorDiameter(turb))
!TODO wakeCentersY(loc, turb) = wakeCentersY(loc, turb)+ &
!TODO & initialWakeDisplacement + displacement
!TODO if (useWakeAngle .eqv. .false.) then
!TODO wakeCentersY(loc, turb) = wakeCentersY(loc, turb) + bd*(deltax)
!TODO end if
!TODO wakeCentersZ(loc, turb) = turbineZ(turb)
!TODO end if
!TODO end do
!adjust k_e to C_T, adjusted to yaw
  ke = ke_in + kecorrct*(ct-region2ct)
...
!TODO zone = 1
! define centerpoint of spline
!TODO zeroloc = turbineXw(turb) - wakeDiameter0/(2.0_dp*ke(turb)*me(zone))
!TODO if (zeroloc + spline_bound*rotorDiameter(turb) < velX(loc)) then ! check this
!TODO wakeDiameters(loc, turb, zone) = 0.0_dp
!TODO else if (zeroloc - spline_bound*rotorDiameter(turb) < velX(loc)) then !check this
!!!!!!!!!!!!!!!!!!!!!! calculate spline values !!!!!!!!!!!!!!!!!!!!!!!!!!
! position of upwind point
!TODO x1 = zeroloc - spline_bound*rotorDiameter(turb)
! diameter of upwind point
!TODO y1 = wakeDiameter0+2.0_dp*ke(turb)*me(zone)*(x1 - turbineXw(turb))
! slope at upwind point
!TODO dy1 = 2.0_dp*ke(turb)*me(zone)
! position of downwind point
!TODO x2 = zeroloc+spline_bound*rotorDiameter(turb)
! diameter at downwind point
!TODO y2 = 0.0_dp
! slope at downwind point
!TODO dy2 = 0.0_dp
! solve for the wake zone diameter and its derivative w.r.t. the downwind
! location at the point of interest
!TODO call Hermite_Spline(x, x1, x2, y1, dy1, y2, dy2, wakeDiameters(loc, turb, zone))
!TODO else if (turbineXw(turb) < velX(loc)) then
!TODO wakeDiameters(loc, turb, zone) = wakeDiameter0 + 2.0_dp*ke(turb)*me(zone)*deltax
!TODO end if
!TODO if (turbineXw(turb) < velX(loc)) then
!TODO zone = 2
!TODO wakeDiameters(loc, turb, zone) = wakeDiameter0 + 2.0_dp*ke(turb)*me(zone)*deltax
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! Velocity !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! initialize velocities in full flow field (optional)
!TODO wsArray = minval(Vinf) !TODO MAY NEED TO FIX THIS?
! apply shear profile to visualization
!     wsArray = wsArray*(velZ/shearZh)**shearCoefficientAlpha
!     print *, wsArray
! initialize axial induction values
