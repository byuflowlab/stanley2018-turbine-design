...
if __name__ == '__main__':
    ...
    d_param = np.random.rand(3)*2.43+3.87
    t_param = np.random.rand(3)*0.01+0.01

    rotorDiameter = np.array([80.])
    turbineZ = np.array([100.])
    ratedPower = np.array([2000.])
    ...
    try:
        os.makedirs('layout/%s'%shearExp)
    except OSError:
        if not os.path.isdir('layout/%s'%shearExp):
            raise

    try:
        os.makedirs('layout/%s/%s'%(shearExp,spacing_multiplier))
    except OSError:
        if not os.path.isdir('layout/%s/%s'%(shearExp,spacing_multiplier)):
            raise
    ...
    prob.driver.opt_settings['Summary file'] = 'layout/%s/%s/summary_%s_%s.out'%(shearExp,spacing_multiplier,jobID,taskID)
    prob.driver.opt_settings['Print file'] = 'layout/%s/%s/verify_%s_%s.out'%(shearExp,spacing_multiplier,jobID,taskID)
    ...
    np.savetxt('layout/%s/%s/XYZ_%s_%s.txt'%(shearExp,spacing_multiplier,jobID, taskID), np.c_[prob['turbineX'], prob['turbineY'], prob['turbineZ']], header="turbineX, turbineY, turbineZ")
    np.savetxt('layout/%s/%s/XYstart_%s_%s.txt'%(shearExp,spacing_multiplier,jobID, taskID), np.c_[turbineXstart, turbineYstart], header="turbineXstart, turbineYstart")
    np.savetxt('layout/%s/%s/diameter_%s_%s.txt'%(shearExp,spacing_multiplier,jobID, taskID), np.c_[diameters], header="diameters")
    np.savetxt('layout/%s/%s/thickness_%s_%s.txt'%(shearExp,spacing_multiplier,jobID, taskID), np.c_[thicknesses], header="thicknesses")
    np.savetxt('layout/%s/%s/COE_%s_%s.txt'%(shearExp,spacing_multiplier,jobID, taskID), np.c_[prob['COE'], prob['AEP']])
    np.savetxt('layout/%s/%s/power_diam_%s_%s.txt'%(shearExp,spacing_multiplier,jobID, taskID), np.c_[ratedPowers,rotorDiameters])
