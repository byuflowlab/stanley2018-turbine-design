...
if __name__ == '__main__':
    ...
    turbineXpert = np.random.rand(nTurbs)*rotor_diameter*5.6 - 2.8*rotor_diameter
    turbineYpert = np.random.rand(nTurbs)*rotor_diameter*5.6 - 2.8*rotor_diameter
    ...
    try:
        os.makedirs('everything_1')
    except OSError:
        if not os.path.isdir('everything_1'):
            raise

    try:
        os.makedirs('everything_1/%s'%shearExp)
    except OSError:
        if not os.path.isdir('everything_1/%s'%shearExp):
            raise

    try:
        os.makedirs('everything_1/%s/%s'%(shearExp,spacing_multiplier))
    except OSError:
        if not os.path.isdir('everything_1/%s/%s'%(shearExp,spacing_multiplier)):
            raise
    ...
    prob.driver.opt_settings['Summary file'] = 'everything_1/%s/%s/summary_%s_%s.out'%(shearExp,spacing_multiplier,jobID,taskID)
    prob.driver.opt_settings['Print file'] = 'everything_1/%s/%s/verify_%s_%s.out'%(shearExp,spacing_multiplier,jobID,taskID)
    ...
    np.savetxt('everything_1/%s/%s/XYZ_%s_%s.txt'%(shearExp,spacing_multiplier,jobID, taskID), np.c_[prob['turbineX'], prob['turbineY'], prob['turbineZ']], header="turbineX, turbineY, turbineZ")
    np.savetxt('everything_1/%s/%s/XYstart_%s_%s.txt'%(shearExp,spacing_multiplier,jobID, taskID), np.c_[turbineXstart, turbineYstart], header="turbineXstart, turbineYstart")
    np.savetxt('everything_1/%s/%s/diameter_%s_%s.txt'%(shearExp,spacing_multiplier,jobID, taskID), np.c_[diameters], header="diameters")
    np.savetxt('everything_1/%s/%s/thickness_%s_%s.txt'%(shearExp,spacing_multiplier,jobID, taskID), np.c_[thicknesses], header="thicknesses")
    np.savetxt('everything_1/%s/%s/COE_%s_%s.txt'%(shearExp,spacing_multiplier,jobID, taskID), np.c_[prob['COE'], prob['AEP']])
    np.savetxt('everything_1/%s/%s/power_diam_%s_%s.txt'%(shearExp,spacing_multiplier,jobID, taskID), np.c_[ratedPowers,rotorDiameters])
