...
if __name__ == '__main__':
    ...
    try:
        os.makedirs('baseline/%s'%shearExp)
    except OSError:
        if not os.path.isdir('baseline/%s'%shearExp):
            raise

    try:
        os.makedirs('baseline/%s/%s'%(shearExp,spacing_multiplier))
    except OSError:
        if not os.path.isdir('baseline/%s/%s'%(shearExp,spacing_multiplier)):
            raise
    ...
    prob.driver.opt_settings['Summary file'] = 'baseline/%s/%s/summary_%s_%s.out'%(shearExp,spacing_multiplier,jobID,taskID)
    prob.driver.opt_settings['Print file'] = 'baseline/%s/%s/print_%s_%s.out'%(shearExp,spacing_multiplier,jobID,taskID)

    prob.driver.add_objective('COE', scaler=1.E0)

    # prob.driver.add_desvar('turbineX', scaler=0.1)
    # prob.driver.add_desvar('turbineY', scaler=0.1)

    for i in range(nGroups):
        prob.driver.add_desvar('d_param%s'%i, lower=3.87, upper=6.3, scaler=1.E-1)
        prob.driver.add_desvar('t_param%s'%i, lower=0.001, upper=None, scaler=10.)
    ...
    np.savetxt('baseline/%s/%s/XYZ_%s_%s.txt'%(shearExp,spacing_multiplier,jobID, taskID), np.c_[prob['turbineX'], prob['turbineY'], prob['turbineZ']], header="turbineX, turbineY, turbineZ")
    np.savetxt('baseline/%s/%s/diameter_%s_%s.txt'%(shearExp,spacing_multiplier,jobID, taskID), np.c_[diameters], header="diameters")
    np.savetxt('baseline/%s/%s/thickness_%s_%s.txt'%(shearExp,spacing_multiplier,jobID, taskID), np.c_[thicknesses], header="thicknesses")
    np.savetxt('baseline/%s/%s/COE_%s_%s.txt'%(shearExp,spacing_multiplier,jobID, taskID), np.c_[prob['COE'], prob['AEP']])
    np.savetxt('baseline/%s/%s/power_diam_%s_%s.txt'%(shearExp,spacing_multiplier,jobID, taskID), np.c_[ratedPowers,rotorDiameters])
